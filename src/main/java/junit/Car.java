...
public class Car {
    ...
    public boolean startOn = false;//añado un booleano para controlar que el coche esté encendido
    ...
    //metodos
    public void start() {
        if (!this.isTachometerGreaterThanZero() && !this.startOn) {
        //if (!this.isTachometerGreaterThanZero() && !this.startOn) {
            this.tachometer = 1000;//TRAMPA, antes asignaba 0 y daba error debido a como se gestiona el tacometro
            this.startOn = true;
        ...
        if (startOn) {
            if (this.speedometer == MAX_SPEED) {
                ...
                if (this.isReverse() && (this.speedometer == 25)) {
                    ...
                    this.speedometer += 5;
                    System.out.println("Acelerouse 5 Km/h máis");
                    if (!this.isReverse()) {
    ...
    public void turnAngleOfWheels(int angle) {
        if (((this.wheelsAngle + angle) <= 45) && ((this.wheelsAngle + angle) >= -45)) {
            ...
            System.out.println("As rodas acaban de xirar " + angle + " grados");
        } else {
            this.wheelsAngle = angle > 0 ? Math.min(45, this.wheelsAngle + angle)
                    : Math.max(-45, this.wheelsAngle + angle);
    ...
    public String showSteeringWheelDetail() {
        ...
        if (this.wheelsAngle == 0) {
            ...
            builder.append(Math.abs(this.wheelsAngle));
            builder.append("º)");
        }
    ...
    public void showDetails() {
        System.out.println("O " + this.brand + " modelo " + this.model + " de " + this.fuel + ". Velocímetro: " + this.speedometer
...
//Solución Alberto(casi, casi idéntica a la mia, salvo por algun detalle en accelerate y brake)
   /*
    ...
    public void brake() {
        if (this.speedometer == 0) {
            ...
            this.speedometer -= 5;
            System.out.println("O vehículo reduciu a velocidade 5 Km/h. A velocidade agora é " + this.speedometer);
            if ((this.speedometer != 0) && !this.isReverse()) {
                this.calculateTachometer();
            } else {
                if (!this.isReverse()) {
    ...
    public void showDetails() {
        System.out.println("O " + this.brand + " " + this.model + " marca: Velocímetro: " + this.speedometer
                + " - Tacógrafo: " + this.tachometer + " - Marcha: " + this.gear + " - Volante: " + this.showSteeringWheelDetail());
    }
